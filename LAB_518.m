...
% at 6 months ( |x| = 0.25 ) there is no clear evidence if it is noise or
% not
N = length(h_obs);
if (mod(N,2)==0) %even   
...
title("Fourier transform (power spectra)" + n_pz)
xlabel('frequency'),grid on
ylim([0,1])
deg = sprintf('obs signal - a0 = %g',max(abs(Fobs1)));
legend(deg,'Location','best')
% M = 2;
% [a_m, b_m] = DFT_analysis(f_h,h_obs, length(h_obs), M);
% [s_interp] = DFT_synthesis(a_m, b_m, x_syn, length(x_syn), M);
% figure,plot(s_dt,s_interp,'-r'),grid on,title('est signal M=2')
% hold on,plot(h_dt,f_h,'or')
